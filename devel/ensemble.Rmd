...
```{r}
oobControl = trainControl(method = "oob", verbose = FALSE)
cvControl = trainControl(method = "cv", number = 5, verbose = FALSE)
```

To tune a Random Forest in `caret` we will use `method = "rf"` which uses the `randomForest` function in the background. Here we elect to use the OOB training control that we created. We could also use Cross-Validation, however it will likely select a similar model, however, requiring more time.
...
set.seed(825)
rfFit = train(Sales ~ ., data = seat_trn,
               method = "rf",
               trControl = oobControl,
               verbose = FALSE,
               tuneGrid = rfGrid)
rfFit
getACC(predict(rfFit, seat_tst), seat_tst$Sales)
```
...
Based on these results, we would select the random forest model with an `mtry` of `r as.numeric(rfFit$bestTune)`. Note that based on the OOB estimates, the bagging model is expected to perform worse than this select model, however, based on our results above, that is not what we find to be true in our test set.
...
```{r, message=FALSE, warning=FALSE}
gbmFit = train(Sales ~ ., data = seat_trn,
                method = "gbm",
                trControl = cvControl,
                verbose = FALSE,
                tuneGrid = gbmGrid)
...
#gbmFit
plot(gbmFit)
getACC(predict(gbmFit, seat_tst), seat_tst$Sales)
```
