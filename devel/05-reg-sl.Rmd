...
Frequently we will take a dataset and split it in two. One part of the datasets will be used to fit (train) a model, which we will call the **training** data. The remainder of the original data will be used to assess how well the model is predicting, which we will call the **test** data. Test data should *never* be used to train a model.
...
# train MSE
sqrt(mean((train_data$Sales - predict(fit, newdata = train_data)) ^ 2))

# test MSE
sqrt(mean((test_data$Sales - predict(fit, newdata = test_data)) ^ 2)) 
```
...
# train MSE
get_mse(actual = train_data$Sales, 
        predicted = predict(fit, newdata = train_data))
# test MSE
get_mse(actual = test_data$Sales, 
