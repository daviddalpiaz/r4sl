...
$$
\text{RMSE}(\hat{f}, \text{Data}) = \sqrt{\frac{\displaystyle\sum_{i = 1}^{n}\left(y_i - \hat{f}(x_i)\right)^2}{n}}
$$
...
$$
\text{RMSE}_{\text{Train}} = \text{RMSE}(\hat{f}, \text{Train Data}) = \sqrt{\frac{1}{n_{\text{Train}}}\displaystyle\sum_{i \in \text{Train}}^{}\left(y_i - \hat{f}(x_i)\right)^2}
$$

Train RMSE will still always go down (or stay the same) as the complexity of a linear model increases. That means train RMSE will not be useful for comparing models, but checking that it decreases is a useful sanity check.

$$
\text{RMSE}_{\text{Test}} = \text{RMSE}(\hat{f}, \text{Train Data}) = \sqrt{\frac{1}{n_{\text{Test}}}\displaystyle\sum_{i \in \text{Test}}^{}\left(y_i - \hat{f}(x_i)\right)^2}
$$

Test RMSE uses the model fit using the training data, but evaluated on the unused test data. What happens to test RMSE as the size of the model increases? That is what we will investigate.


We'll start with the simplest possible linear model, that is, a model with no predictors.
