...
We will now use simulation to estimate the bias, variance, and mean squared error for the estimates for $f(x)$ given by these models at the point $x_0 = 0.9$. We use simulation to complete this task, as performing the exact calculations are always difficult, and often impossible.
...
The above plot shows the 1000 trained models for each of the four models considered. The truth is given by the black and white dashed line.


## Bias-Variance Tradeoff

To evaluate the bias and variance, we simulate values for the response $y$ at $x_0 = 0.9$ according to the true model.
...
When then use the predictions obtained from the above simulation to estimate the bias, variance and mean squared error for estimating $f(x)$ at $x_0 = 0.9$ for the four models.
...
- We use squared bias in this table. Since bias can be positive or negative, squared bias is more useful for observing the trend as complexity increases. That trend, which we see here, is **decreasing** bias as complexity increases, which we expect to see in general.
...
- TODO: MSE missing noise in table
